...
from ode4jax.base import AbstractIntegrator, AbstractSolution
from ode4jax.base import alg_cache, default_norm, calculate_error
...
def _init(
    ...
    *,
    ...
    """
    Constructs an ODEIntegrator.

    Arguments:
            problem: The ODEProblem to be integrated.
            alg: The AbstractODEAlgorithm to use.

    Additional Arguments:
            abstol: Absolute tolerance to be used with adaptive timestepping
            reltol: Relative tolerance to be used with adaptive timestepping

            dt: Initial timestep (mandatory if non-adaptive algorithm is used)
            dtmin: ...
            dtmax: ...
            maxiters: Maximum number of iterations

            controller: Controller to be used to compute the timestep during the integration. Defaults to PIController.
            qmin, qmax, qsteady_min, qsteady_max, gamma: Variables controlling the controllers.

            errornorm:
            internalnorm: a function `f(u, t)` that returns the inner norm of vector u at time t.
            errornorm: a function that returns the error estimate for a step.

            save_start: save the configuration before the first timestep.
            save_end: save the configuration after the last timestep.
            save_everystep: ignored
            saveat: save the configuration at a list of times. If it is an integer, it subdivides the times into a range
                    with that many sub-steps.
    """
    ...
    if save_start is None:
        save_start = (
            save_everystep or len(_saveat) == 0 or np.isscalar(saveat)
        )  # or problem.tspan[0] in saveat
        _saveat = jnp.append(tspan[0], _saveat)

    if save_end is None:
        save_end = (
            save_everystep or len(_saveat) == 0 or np.isscalar(saveat)
        )  # or problem.tspan[1] in saveat
        _saveat = jnp.append(_saveat, tspan[1])

    opts = DEOptions(
        maxiters=maxiters,
        adaptive=adaptive,
        abstol=abstol_internal,
        reltol=reltol_internal,
        controller=controller,
        saveat=jnp.sort(_saveat),
        next_saveat_id=0,
        tstops=jnp.sort(tstops),
...
def _initialize(integrator: ODEIntegrator):
    if integrator.opts.save_start:
        ...
        integrator.opts.next_tstop_id = jax.lax.cond(
            cond,
        ...
        integrator.opts.next_saveat_id = jax.lax.cond(
            cond,
    ...
    integrator.fsalfirst = integrator.f(
        integrator.uprev, integrator.p, integrator.t
    )  # Pre-start fsal
    integrator.fsallast = jax.tree_map(
        lambda x: jnp.zeros_like(x), integrator.fsalfirst
    )
